...
static json_t *handle_list_sexp_categories()
{
	...
	for (int extra_id : extra_categories) {
		...
		if (count > 0) {
			json_t *cat = json_object();
			json_object_set_new(cat, "name", json_string(mcp_get_sexp_category_name(extra_id)));

			const char *desc = mcp_get_sexp_category_description(extra_id);
			if (desc)
				json_object_set_new(cat, "description", json_string(desc));
...
static json_t *handle_list_sexp_operators(json_t *arguments)
{
	...
	// Phase 1: collect candidates that pass category/subcategory filters
	SCP_vector<std::pair<size_t, const char *>> candidates;
	for (size_t i = 0; i < Operators.size(); i++) {
		const auto &op = Operators[i];

		if (filter_category_id >= 0 && get_category(op.value) != filter_category_id)
			continue;
		if (filter_subcategory_id >= 0 && get_subcategory(op.value) != filter_subcategory_id)
			continue;

		candidates.emplace_back(i, op.text.c_str());
	}

	// Phase 2: fuzzy search and sort
	auto matches = fuzzy_search_and_sort(candidates, filter_search, Max_operator_length);
	...
	if (is_variadic) {
		...
		if (cycle_found) {
			...
			for (int a = 0; a < prefix_len; a++)
				json_array_append_new(arg_types, json_string(opf_to_name(types[a])));
			...
			for (int a = prefix_len; a < prefix_len + cycle_len; a++)
				json_array_append_new(var_types, json_string(opf_to_name(types[a])));
		...
		for (int a = 0; a < max_args; a++) {
			...
			if (atype == OPF_NONE)
				break;
			json_array_append_new(arg_types, json_string(opf_to_name(atype)));
		}
		json_object_set_new(obj, "argument_types", arg_types);
	}
}
...
static json_t *handle_list_reference_notes(json_t *arguments)
{
	...
	// Phase 1: collect candidates that pass category filter
	SCP_vector<std::pair<size_t, const char *>> candidates;
	size_t index;
	json_t *entry;
	...
	// Phase 2: fuzzy search and sort
	auto matches = fuzzy_search_and_sort(candidates, filter_search);
...
static json_t *handle_get_sexp_argument_type(json_t *arguments)
{
	...
	if (!name) {
		json_t *arr = json_array();
		for (auto t = Opf_type_info; t->name; t++) {
			json_t *item = json_object();
	...
	auto info = find_named_info(Opf_type_info, name);
	if (!info)
...
static json_t *handle_get_sexp_return_type(json_t *arguments)
{
	...
	if (!name) {
		json_t *arr = json_array();
		for (auto t = Opr_type_info; t->name; t++) {
			json_t *item = json_object();
			json_object_set_new(item, "name", json_string(t->name));
			json_object_set_new(item, "description", json_string(t->description));
			json_array_append_new(arr, item);
	...
	// Look up the type
	auto info = find_named_info(Opr_type_info, name);
	if (!info)
...
static json_t *handle_list_scripting_elements(json_t *arguments)
{
	...
	// Phase 1: collect candidates that pass type filter, build index of filtered elements
	SCP_vector<size_t> filtered;
	size_t idx;
	json_t *el;
	...
	// Phase 2: fuzzy search across both name and shortName fields (best cost wins)
	size_t max_name_length = std::max(
		json_array_max_string_length(elements, "name"),
		json_array_max_string_length(elements, "shortName"));

	SCP_string search_str(filter_search ? filter_search : "");
	auto matches = fuzzy_search_and_sort(filtered.size(), filter_search,
		[&](size_t fi) -> size_t {
			json_t *e = json_array_get(elements, filtered[fi]);
	...
	json_t *arr = json_array();
	for (const auto &match : matches) {
		json_t *e = json_array_get(elements, filtered[match.first]);
		const char *name = json_string_value(json_object_get(e, "name"));
		const char *shortName = json_string_value(json_object_get(e, "shortName"));
...
static json_t *handle_list_scripting_hooks(json_t *arguments)
{
	...
	// Phase 1: collect candidates that pass overridable filter
	SCP_vector<std::pair<size_t, const char *>> candidates;
	size_t idx;
	json_t *action;
	json_array_foreach(actions, idx, action) {
		...
		candidates.emplace_back(idx, name);
	}
