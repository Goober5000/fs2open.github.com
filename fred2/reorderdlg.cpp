...
void reorder_dlg::swap_slot_contents(int slot_a, int slot_b)
{
	if (ships_selected())
	{
		...
		swap_ship_slots(slot_a, slot_b, cfg);
	}
	else
	{
		...
		swap_wing_slots(slot_a, slot_b, cfg);
	}
}
...
void reorder_dlg::move_selected(bool up, bool all_the_way)
{
	...
	// Bubble the item toward the target one adjacent swap at a time, which
	// preserves the relative order of everything else in the list.
	int step = (target > pos) ? 1 : -1;
	for (int j = pos; j != target; j += step)
		swap_slot_contents(m_slots[j], m_slots[j + step]);
