...
void parse_xwi_objectgroup(mission *pm, const XWingMission *xwim, const XWMObject *oj)
{
	...
	int number_of_objects = oj->numberOfObjects; // needs to be fixed in FSO for large minefields...
	if (number_of_objects > MAX_SHIPS_PER_WING) {
		Warning(LOCATION,
			"Too many ships in Flight Group %s.  FreeSpace supports up to a maximum of %d.",
			fg->designation.c_str(),
			MAX_SHIPS_PER_WING);
		number_of_objects = MAX_SHIPS_PER_WING;
	}

	// see if this flight group is what FreeSpace would treat as a wing... not sure if we need for objects?
	wing* wingp = nullptr;
	int wingnum = -1;
	if (is_wing(fg)) {
		wingnum = Num_wings++;
		...
		if (formation_name) {
			...
			if (wingp->formation < 0)
				Warning(LOCATION,
					"Formation %s from Flight Group %s was not found",
					formation_name,
					fg->designation.c_str());
		}
		if (wingp->formation >= 0 && !is_fighter_or_bomber(fg))
	...
	// all ships in the flight group share a class, so determine that here..... not sure if we need for objects
	int ship_class = xwi_determine_ship_class(fg);
	if (ship_class < 0) {
		Warning(LOCATION, "Unable to determine ship class for Flight Group %s", fg->designation.c_str());
		ship_class = 0;
	...
	// similarly for the team....... needed for objects? Apparently in XW the iff isn't utilised for objects??
	auto team_name = xwi_determine_team(xwim, oj, sip);
	int team = Species_info[sip->species].default_iff;
	if (team_name) {
		int index = iff_lookup(team_name);
		if (index >= 0)
	...
	// NOTE: Y and Z are swapped
	auto ojxyz = vm_vec_new(oj->object_x, oj->object_z, oj->object_y);
	auto okPRB = vm_vec_new(fg->start2_x, fg->start2_z, fg->start2_y); //change to yaw pitch roll

	// units are in kilometers (after processing by xwinglib which handles the factor of 160), so scale them up
	vm_vec_scale(&ojxyz, 1000);

	matrix orient; // we have yaw pitch and roll so these can be input directly here for object instead of orient
	xwi_determine_orientation(&orient, fg, &start1, &start2, &start3, &waypoint1, &waypoint2, &waypoint3, &hyperspace);

	// now configure each ship in the flight group..... not sure what needs to be done here exactly
	for (int wing_index = 0; wing_index < number_in_wave; wing_index++) {
		p_object pobj;
		...
		pobj.ship_max_hull_strength = sip->max_hull_strength;
		Assert(
			pobj.ship_max_hull_strength > 0.0f); // Goober5000: div-0 check (not shield because we might not have one)
		pobj.max_shield_recharge = sip->max_shield_recharge;
